...
async sessionStart() {
		...
		if (redirect == null && !devMode) {
			...
			if (isOptionSet('customSkyidUrl', this.opts)) {
				...
				window.windowObjectReference = await popupCenter(
	...
	async getFile(dataKey, callback) {
		await this.getJSON(dataKey, callback)
	}
	async setFile(dataKey, json, callback) {
		await this.setJSON(dataKey, json, callback)
	}
	...
	async getId(){ return this.userId; }
