<template>
  <div class="side-bar__panel side-bar__panel--menu">
    ...
    <div v-else>
      ...
      <div v-for="token in skyIdTokens" :key="token.sub">
        <menu-entry @click.native="openSkyId(token)">
          <icon-provider slot="icon" provider-id="skyId"></icon-provider>
          <div>Open from SkyDB</div>
          <span>{{token.name}}</span>
        </menu-entry>
        <menu-entry @click.native="saveSkyId(token)">
          <icon-provider slot="icon" provider-id="skyId"></icon-provider>
          <div>Save on SkyDB</div>
          <span>{{token.name}}</span>
        </menu-entry>
      </div>
      <hr>
    ...
    async openSkyId(token) {
      try {
        ...
        store.dispatch(
          'queue/enqueue',
          async () => {
            await skyIdProvider.openFile(token, syncLocation);
            badgeSvc.addBadge('openFromSkyId');
          },
        );
      } catch (e) { /* cancel */ }
    },
    async saveSkyId(token) {
      try {
        await openSyncModal(token, 'skyIdSave');
        badgeSvc.addBadge('saveOnSkyId');
      } catch (e) { /* cancel */ }
    },
